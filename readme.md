...
预约成功、失败或浏览器流程异常结束时，程序会在项目根目录的 `diagnostics/` 生成一份 `checkout-*.txt` 浏览器响应快照，并在终端输出文件路径。快照包含 `saveOrder` 的 HTTP 状态、响应中的业务码和提示；如未收到该接口响应，也会明确注明。文件不会记录请求参数、响应的完整 `data`、登录凭据或验证码数据，`diagnostics/` 已被 Git 忽略。排查时可比较 HTTP 状态、业务码和提示，并在重试前先确认订单列表中是否已生成订单。
